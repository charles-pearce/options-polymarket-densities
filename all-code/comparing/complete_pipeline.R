...
get_results = function(data_loc, method, calibration_coeff = list("COMBINED" =c(1,1,1,1)), 
  ...
  # ignore na values (happens when a date is skipped because of results)
  density_df = result |>
    keep(~ !is.null(.x$density)) |>
    map(~ .x$density |> mutate(DTE = as.integer(DTE))) |>
    bind_rows()
  rownames(density_df) = NULL
  
  probability_df = result |>
    keep(~ !is.null(.x$probability)) |>
    map(~ .x$probability |> mutate(DTE = as.integer(DTE))) |>
    bind_rows()
  
  skipped = bind_rows(map(result, "skipped"))
  num_skipped_strikes = mean(unlist(map(result, "skipped_option_strikes")), na.rm = TRUE)
...
combined_results = function(name, data_loc, method, calibration_coeff, dx = 0.1){
  ...
  for(cal_name in names(this_coeff)){
    ...
    for(dte in setdiff(unique(option_data$DTE), 0)){
      ...
      attempt_probability = try({
        this_probability = get_both_probabilities(option_fit = option_density, 
                               this_pm_data = this_pm_data,
                               dte = dte,
                               stock = stock,
                               date = date,
      ...
      # if there was an error in extracting the probabilities, add it to the 
      # skipped dates
      if (inherits(attempt_probability, "try-error")) {
        skipped_list[[length(skipped_list)+1]] = data.frame(
          "stock" = stock, 
          "date" = date, 
          "dte" = dte,
          "calibration_level" = cal_name,
          "reason" = "Couldn't extract probabilities",
          "error" = attempt_probability[1]
        )
        next
      } else {
        probability[[cal_name]][[dte]] = this_probability
      ...
      this_density_attempt = try({
        this_density = get_comparison_density(option_density = option_density, 
      ...
      if (inherits(this_density_attempt, "try-error")) {
        skipped_list[[length(skipped_list)+1]] = data.frame(
          "stock" = stock, 
          "date" = date, 
          "dte" = dte,
          "calibration_level" = cal_name,
          "reason" = "Couldn't compare densities",
          "error" = this_density_attempt[1]
        )
      ...
      density[[cal_name]][[dte]] = this_density
  ...
  return(list(density = do.call(rbind, lapply(density, function(x) do.call(rbind, x))),
              ...
              skipped_option_strikes = if (exists("option_density")) option_density$num_na else NA)
         )
}
###########################################
get_comparison_density = function(option_density, pm_density, dte, 
  ...
  # make sure pdfs integrate to 1 with our precision
  int_pdf_pm = sum(y_pm) *dx
  int_pdf_opt = sum(y_options)*dx
  if(abs(int_pdf_pm-1) <= 0.05) {
    y_pm = y_pm/int_pdf_pm
  } else {
    stop(paste("pm pdf integrates to:", int_pdf_pm))
  }
  if(abs(int_pdf_opt-1) <= 0.05) {
    y_options = y_options/int_pdf_opt
  } else {
    stop(paste("option pdf integrates to:", int_pdf_opt))
  }
...
range(sapply((strsplit(gsub( "<|>", "",this_pm_data$bracket), "-")),
       function(x) as.numeric(x)))

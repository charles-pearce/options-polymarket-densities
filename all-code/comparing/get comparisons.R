...
# set the seed for the remainder of the code to make it repllicable
set.seed(6969)
...
for(method in c("3lognorm", "nonparam", "1lognorm")){
  ...
  accuracy_result_dte = probabilities |>
    # skip stock specific calibrations
    filter(calibration_method %in% c("COMBINED", "NOT CALIBRATED")) |>
    nest_by(calibration_method) |>
    mutate(result = list(compare_accuracy_by_dte(data))) |>
    unnest(result) |>
    select(-data)
  
  accuracy_result_dte_stock = probabilities |>
    nest_by(stock, calibration_method) |>
    mutate(result = list(compare_accuracy_by_dte(data))) |>
    unnest(result) |>
    select(-data)
  ...
  mz_results_by_DTE = probabilities |>
    filter(calibration_method %in% c("COMBINED", "NOT CALIBRATED")) |>
    nest_by(DTE, calibration_method)|>
    mutate(result = list(test_mz(lm(pm_probability ~ option_probability, data = data))),
           stock = "all") |>
    unnest(result) |>
    select(DTE, coefficient, estimate, F_stat, p_val, stock, calibration_method)
  
  mz_result_by_DTE_stock = probabilities |>
    nest_by(DTE, stock, calibration_method) |>
    mutate(result = list(test_mz(lm(pm_probability ~ option_probability, data = data)))) |>
    unnest(result) |>
    select(DTE, coefficient, estimate, F_stat, p_val, stock, calibration_method)

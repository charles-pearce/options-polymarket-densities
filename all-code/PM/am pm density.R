...
fit_pm_density = function(method, data, educated_guess = NULL, x_range = NULL, dx = NULL){
  ...
  if(method == "nonparam"){
    # if the x_range (coming from the options) is outside the bracket range,
    # use it to restrict the bracket range (which is always (0,Inf)
    if(x_range[1] <= brackets[[1]][2] && x_range[2] >= brackets[[length(brackets)]][2]){
      brackets[[1]][1] = x_range[1]
      brackets[[length(brackets)]][2] = x_range[2]
    } else {
      # otw restrict it by setting the bound to the minimum of going on bracket width further
      # or 0.9*upper bound of lowest bracket and 1.1*lower of highest bracket
      int_width = brackets[[2]][2] - brackets[[2]][1]
      brackets[[1]][1] = min(brackets[[1]][2]- int_width, brackets[[1]][2]*0.9)
      len = length(brackets)
      brackets[[len]][2] = max(brackets[[len]][1] + int_width, brackets[[len]][1] *1.1)
    }
  ...
  this_objective = function(prms) {
    ...
    if(method == "3lognorm"){
      ...
      # restrict sigma to be larger than 1e-4 to prevent one of the lognormals
      # to essentially be a line
      if (any(params[c("sd.1", "sd.2", "sd.3")] < 1e-4)) return(1e10)

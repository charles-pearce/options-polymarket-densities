...
den_df <- bind_rows(
  nc_np |> transmute(model = "Nonparametric",     DTE = factor(DTE),
                     hellinger   = Hellinger.Distance,
                     wasserstein = Wasserstein.Distance),
  nc_1l |> transmute(model = "Single Lognormal",  DTE = factor(DTE),
                     hellinger   = Hellinger.Distance,
                     wasserstein = Wasserstein.Distance),
  nc_3l |> transmute(model = "Lognormal Mixture", DTE = factor(DTE),
                     hellinger   = Hellinger.Distance,
                     wasserstein = Wasserstein.Distance)
) |>
...
# Panel A: Hellinger
p_hell <- ggplot(den_df, aes(x = DTE, y = hellinger)) +
  geom_boxplot(
    fill          = "grey40",
    colour        = "black",
    linewidth     = 0.4,
  ...
  labs(x = "Days Before Expiry", y = "Hellinger Distance", tag = "A") +
  theme_jof +
  theme(legend.position = "none")

# Panel B: Wasserstein — log scale
p_wass <- ggplot(den_df |> mutate(wasserstein = wasserstein + 1e-6),
                 aes(x = DTE, y = wasserstein)) +
  geom_boxplot(
    fill          = "grey40",
    colour        = "black",
    linewidth     = 0.4,
  ...
  facet_wrap(~model, nrow = 1) +
  labs(x = "Days Before Expiry", y = "Wasserstein Distance (log scale)",
       tag = "B") +
  theme_jof +
  theme(legend.position = "none")
...
ggsave("fig_hellinger.png", fig, width = 6.5, height = 6.2, dpi = 300)